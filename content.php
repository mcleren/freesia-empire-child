...
global $post; ?>
	...
	<article <?php post_class('post-format'.' format-'.$format); ?> id="post-<?php the_ID(); ?>">
		...
		<?php 
			...
			<?php /*<span class="author vcard"><?php _e('Author :','freesia-empire')?><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php the_author(); ?>"><?php the_author(); ?> </a></span> */?>
			<span class="posted-on"><?php _e('','freesia-empire')?><a title="<?php echo esc_attr( get_the_time() ); ?>" href="<?php the_permalink(); ?>"><?php the_time( get_option( 'date_format' ) ); ?></a></span>
			<span class="cat-links"><?php _e('Category | ','freesia-empire');  the_category(', '); ?></span> <!-- end .cat-links -->
