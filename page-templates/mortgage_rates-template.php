<?php
/**
 * Template Name: Mortgage Rates Template
 *
...
if (file_exists($inputFileName) && ($handle = fopen($inputFileName, "r")) !== FALSE) {
	...
	if(isset($disclaimer[1])){
		$current_time = current_time( 'timestamp', 0 ) - 36000;
		if (date('N', $current_time) == 6) // Show Friday' date on Saturday
			$current_time -= 86400;
		else if (date('N', $current_time) == 7) // Show Friday' date on Sunday
			$current_time -= 172800;
		$disclaimer[1] = str_replace('[filemtime]', date("F d, Y", $current_time), $disclaimer[1]); // Replace [filemtime] with today' date after 10:00 AM
		//$disclaimer[1] = str_replace('[filemtime]', date("F d, Y", filemtime($inputFileName)), $disclaimer[1]); // Replace [filemtime] with file updated date
	}
}
//print_r($rates);
?>
		...
		<div id="rates-box">
			<div class="clearfix">
				<div>
					...
					?>
					<div style="clear: left;padding-top: 5px;" class="tab-visible-static">Like what you see? Get your customized rate within minutes! Call <?php echo lfm_campaign_toolfree_ajx();?> or Get your free quote <a href="/mortgage-quick-quote/" title="Mortgage Quick Quote">NOW</a>.</div>
				</div>
			</div>
		</div>	
	</div>
