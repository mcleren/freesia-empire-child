	...
	if( have_posts() ) {
		...
		<div class="entry-content">
			<?php 
				...
				*
				*/
				$results = $wpdb->get_results( "SELECT * FROM `lfm_testimonials` WHERE `status`='Published' ORDER BY `date` DESC", ARRAY_A);

				if ( $results ) {
				    foreach ( $results as $result ){
						echo '<blockquote>' . apply_filters('the_content', $result['description']);
						echo '<cite>— ' . $result['author'] . '</cite>';
						echo '</blockquote>';						
					}
				}
