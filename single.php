...
<div id="primary">
	...
	<main id="main" class="site-main clearfix"><!-- Using child/blog.php	-->
			...
			<article <?php post_class('post-format'.' format-'.$format); ?><?php  ?> id="post-<?php the_ID(); ?>">
				...
				if($entry_format_meta_blog == 'show-meta' ){?>
					<div class="entry-meta">
						...
						<?php /*<span class="author vcard"><?php _e('Author :','freesia-empire')?><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php the_author(); ?>"><?php the_author(); ?> </a></span> <!-- .author vcard -->*/?>
						<span class="posted-on"><?php _e('','freesia-empire')?><a title="<?php echo esc_attr( get_the_time() ); ?>" href="<?php the_permalink(); ?>"><?php the_time( get_option( 'date_format' ) ); ?></a></span> <!-- .posted-on -->
						<span class="cat-links"><?php _e('Category | ','freesia-empire');  the_category(', '); ?> </span> <!-- .cat-links -->						
